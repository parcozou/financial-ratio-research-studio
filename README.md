...
The app requires Python 3.11 or later. Python 3.12 is recommended for Streamlit Community Cloud deployment. Users should install the packages listed in `requirements.txt`, including Streamlit, WRDS, pandas, NumPy, matplotlib, openpyxl, OpenAI, psycopg2-binary, and tabulate. A valid WRDS account is required to retrieve Compustat - North America data.
...
The public repository does not include a real `.streamlit/secrets.toml` file. This keeps private WRDS and LLM API credentials safe. A cloned copy can still run locally without secrets: users can enter their WRDS credentials and, for online AI summaries, enter their own OpenAI-compatible API key in the app interface. Users who prefer local AI can choose LM Studio instead.

If you want to configure your own local default online API key, copy `.streamlit/secrets.toml.example` to `.streamlit/secrets.toml` and fill in your own key.
...
When a shared online LLM key is configured in Streamlit secrets, the app limits shared-key AI summaries to three requests per user connection per day by default. If no shared key is configured, users must enter their own API key, and your private quota is not used.
...
[Public Demo Video - Google Drive](https://drive.google.com/file/d/1GUSZ8eNrqhVE8NLbNIfZnquxohWoZwiY/view?usp=sharing)

[XJTLU Demo Video - Mediasite](https://video.xjtlu.edu.cn/Mediasite/Play/731fd276a96148ffa8cfc0d4ec7109d11d)
