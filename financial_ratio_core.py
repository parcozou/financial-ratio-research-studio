...
LMSTUDIO_TIMEOUT_SECONDS = 600.0
LMSTUDIO_MAX_RETRIES = 0
ACCESS_DATE_TIMEZONE = ZoneInfo("Asia/Shanghai")
...
def format_access_date() -> str:
    access_date = datetime.now(ACCESS_DATE_TIMEZONE)
    return f"{access_date.day} {access_date.strftime('%B')} {access_date.year}"


def build_source_line(source_text: str = "Compustat - North America") -> str:
    source_text = str(source_text).strip()
    if "accessed:" not in source_text.lower():
        source_text = f"{source_text} (Accessed: {format_access_date()})"
    return f"Source(s): {source_text}"
...
def build_metric_texts(
    ...
    source = build_source_line(source_text)
...
def build_http_timeout(timeout_seconds: float) -> httpx.Timeout:
    ...
    connect_timeout = min(20.0, timeout_seconds)
...
def create_lmstudio_client(
    ...
    timeout_seconds: float = LMSTUDIO_TIMEOUT_SECONDS,
    max_retries: int = LMSTUDIO_MAX_RETRIES,
) -> OpenAI:
