...
def get_optional_secret_value(name: str, default: str = "") -> str:
    value = get_secret_value(name, default)
...
def get_online_llm_defaults() -> dict:
    return {
        ...
        "api_key": get_optional_secret_value("DEFAULT_ONLINE_LLM_API_KEY", ""),
...
def build_lmstudio_error_message(
    exc: Exception,
    *,
    model_id: str = "",
    ...
    if any(marker in lowered for marker in ["timed out", "timeout", "read timeout"]):
        wait_minutes = max(int(round(timeout_seconds / 60)), 1)
        details.append(
            f"LM Studio did not return a complete response within about {wait_minutes} minutes. "
            "The local model may still finish inside LM Studio, but the app stopped waiting for the reply."
        )
        details.append(
            "Try selecting fewer metrics, reducing the requested output length, "
            "using a smaller/faster local model, or switching to the online API option."
        )
    elif any(marker in lowered for marker in ["connection error", "connect", "localhost", "refused"]):
        details.append(
            "Please make sure LM Studio is open, the local server is running, "
            "and a model is loaded before requesting the summary."
        )
    ...
    with left_col:
        ...
        if ai_provider == "LM Studio (local)":
            ...
            online_api_key = st.text_input(
                ...
                key="online_api_key",
                placeholder=(
                    "Optional: blank uses shared key."
                    if online_defaults["api_key"]
                    else "Required: enter your own key."
                ),
                help=(
                    "If a shared server-side key is configured by the app owner, leaving this blank will use that key. "
                    "Public GitHub clones do not include a real key, so local users can enter their own key here."
                ),
            )
            if online_provider_preset == "Custom OpenAI-compatible API":
                ...
                st.caption(
                    "No shared online API key is bundled with the public code. "
                    "Enter your own key, or use LM Studio in local mode."
                )
            ...
            if form_values["ai_provider"] == "LM Studio (local)":
                with st.spinner("Asking LM Studio for a compact summary..."):
                    try:
                        ...
                        analysis_error = build_lmstudio_error_message(
                            exc,
                            model_id=form_values["preferred_model"],
                            base_url=form_values["lmstudio_url"],
                        )
...
<div class="hint-card">
    ...
    if not report["pivot_tables"]:
        ...
        with raw_tabs[0]:
            if report["company_raw"].empty:
                ...
                st.dataframe(report["company_raw"], width="stretch", hide_index=True)
        with raw_tabs[1]:
            if report["industry_raw"].empty:
                st.info("No industry benchmark rows were retrieved for this query.")
            else:
                st.dataframe(report["industry_raw"], width="stretch", hide_index=True)
        return
    ...
    with exports_tab:
        if cloud_mode:
            ...
            with export_cols[0]:
                if st.button("Save Excel tables locally", width="stretch"):
            ...
            with export_cols[1]:
                if st.button("Save SVG charts locally", width="stretch"):
            ...
            with export_cols[2]:
                if st.button("Save markdown package locally", width="stretch"):
    ...
    with overview_tab:
        ...
        with raw_tabs[0]:
            st.dataframe(report["long_table_display"], width="stretch", hide_index=True)
        with raw_tabs[1]:
            if report["company_raw"].empty:
                st.info("No company fundamentals were returned from WRDS.")
            else:
